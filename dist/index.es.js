import React, { forwardRef, useState, useRef, useDebugValue, useEffect, useCallback, useLayoutEffect, useImperativeHandle, Children, isValidElement, Fragment, cloneElement } from 'react';
...
const ToolTip = /*#__PURE__*/forwardRef((props, ref) => {
  ...
  useDebugValue(mount.current, value => value ? "Tooltip is mounted" : "Tooltip is unmounted");
  useDebugValue(show, value => value ? "Tooltip is visible" : "Tooltip is hidden");
  useDebugValue(rect, value => `rect: ${JSON.stringify(value)}`);
  useDebugValue(state, value => `state: ${JSON.stringify(value)}`);
  useDebugValue(showTimeoutRef.current, value => `showTimeoutRef: ${value}`);
  useDebugValue(hideTimeoutRef.current, value => `hideTimeoutRef: ${value}`);
  const visibile = useRef(false);
  useEffect(() => {
    mount.current = true;
    return () => {
      mount.current = false;
  ...
  const pastShow = useCallback(hoverRect => {
  ...
  let arrowStyle = state.type === "top" || state.type === "bottom" ? {
    left: state.arrow
  } : {
    top: state.arrow
  };
  ...
  const handleShowTooltip = useCallback(evt => {
  ...
  const handleClearTooltip = useCallback(forceChangedelay => {
  ...
  useEffect(() => {
    ...
    const handleOutsideClick = e => {
      if (childRef?.current && !childRef.current.contains(e.target) && (props.trigger.includes("click") || props.trigger.includes("touch"))) {
        handleClearTooltip();
      }
    };
  ...
  } else if (typeof props.content === "object" && props.content.type === "image") {
  ...
  } else if (typeof props.content === "object" && props.content.type === "list") {
    content = /*#__PURE__*/React.createElement("ul", {
      className: "tooltip-list"
    }, props.content.items.map(item => /*#__PURE__*/React.createElement("li", {
      key: item
    }, item)));
  } else {
    content = props.content;
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, Children.map(props.children, (child, index) => {
    if (index === 0 && /*#__PURE__*/isValidElement(child)) {
      if (child.type !== Fragment) {
        return /*#__PURE__*/cloneElement(child, addPropsAndEventHandlers(child, props));
      } else {
        return /*#__PURE__*/React.createElement("div", _extends({}, show && {
          "aria-describedby": "tooltip",
          "data-testid": "tooltip-trigger"
        }, addEventHandlers(child, props)), /*#__PURE__*/cloneElement(child, {
          ...child.props
        }));
      }
    }
    return child;
  }), mount.current && show && props.show && /*#__PURE__*/createPortal( /*#__PURE__*/React.createElement("div", {
    id: "tooltip",
    ...
    tabIndex: 0,
    "data-testid": "tooltip",
    "aria-label": props.content === "string" ? props.content : "tooltip"
